...
export default function History() {
  const { history, savedBuyers, loading, dataError, refresh } = useApp()
  const [query, setQuery] = useState('')
  const [category, setCategory] = useState('all')
  const [sort, setSort] = useState('newest')
  const [deleting, setDeleting] = useState(null)
  const savedPurchases = useMemo(() => savedBuyers.flatMap((buyer) => buyer.history.map((purchase, index) => ({
    ...purchase, user_id: buyer.user_id, key: `${buyer.user_id}-${index}`,
  }))), [savedBuyers])
  const categories = [...new Set([
    ...history.map((item) => item.purchased_product.category),
    ...savedPurchases.map((item) => item.category),
  ])]
  ...
  const filteredSaved = useMemo(() => savedPurchases.filter((item) => {
    const haystack = `${item.user_id} ${item.product} ${item.category}`.toLowerCase()
    return haystack.includes(query.toLowerCase()) && (category === 'all' || item.category === category)
  }), [savedPurchases, query, category])
  ...
  const noMatches = !filtered.length && !filteredSaved.length

  return <div className="page">
    <PageHeader eyebrow="Activity" title="Purchase history" description="Search entered buyer purchases and inspect, export, or remove dated simulated transactions." action={<div className="button-row"><button className="btn btn-secondary" onClick={exportJson}><FileJson size={17}/>Export JSON</button><button className="btn btn-secondary" onClick={exportCsv}><Download size={17}/>Purchase CSV</button></div>}/>
    <Card className="filter-bar"><label className="search-box"><Search/><span className="sr-only">Search history</span><input placeholder="Search buyer, product, or category…" value={query} onChange={(event) => setQuery(event.target.value)}/></label><label><SlidersHorizontal/><span className="sr-only">Filter category</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All categories</option>{categories.map((item) => <option value={item} key={item}>{formatCategory(item)}</option>)}</select></label><label><span className="sr-only">Sort order</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></label></Card>
    {filtered.length > 0 && <div className="stack-lg"><div className="section-heading"><div><p className="eyebrow">Recorded activity</p><h2>Simulated transactions</h2></div></div><div className="timeline">{filtered.map((item) => <Card className="timeline-item" key={item.transaction.transaction_id}><span className="timeline-dot"/><div className="timeline-date"><strong>{dateTime(item.transaction.purchased_at)}</strong><span>{item.user_id}</span></div><div className="timeline-content"><div><Badge tone="accent">{formatCategory(item.recommendation.category)}</Badge><h2>{item.purchased_product.product}</h2><p>{item.recommendation.reason}</p></div><div className="timeline-purchase"><strong>{money(item.purchased_product.price)}</strong><small>{item.recommendation_metrics ? `${item.recommendation_metrics.confidence}% confidence · ${(item.recommendation_metrics.thinking_duration_ms / 1000).toFixed(2)}s` : 'Legacy record · timing not recorded'}</small></div></div><button className="btn-icon danger" aria-label={`Delete ${item.transaction.transaction_id}`} onClick={() => setDeleting(item)}><Trash2/></button></Card>)}</div></div>}
    {filteredSaved.length > 0 && <div className="stack-lg"><div className="section-heading"><div><p className="eyebrow">Saved profiles</p><h2>Entered purchase history</h2><p>These purchases came from buyer profiles created in this browser, so transaction dates were not recorded.</p></div></div><div className="timeline">{filteredSaved.map((item) => <Card className="timeline-item" key={item.key}><span className="timeline-dot"/><div className="timeline-date"><strong>Not recorded</strong><span>{item.user_id}</span></div><div className="timeline-content"><div><Badge>{formatCategory(item.category)}</Badge><h2>{item.product}</h2><p>Imported as prior buyer history.</p></div><div className="timeline-purchase"><strong>{money(item.price)}</strong><small>Saved buyer profile</small></div></div></Card>)}</div></div>}
    {noMatches && <EmptyState title="No matching history" description="Adjust the filters or create a buyer with purchase history."/>}
    {deleting && <div className="dialog-backdrop" onMouseDown={() => setDeleting(null)}><div className="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-title" onMouseDown={(event) => event.stopPropagation()}><span className="danger-icon"><Trash2/></span><h2 id="delete-title">Delete this interaction?</h2><p>This removes transaction {deleting.transaction.transaction_id} and rebuilds the buyer’s latest memory snapshot.</p><div className="button-row"><button className="btn btn-secondary" onClick={() => setDeleting(null)}>Cancel</button><button className="btn btn-danger" onClick={() => remove(deleting)}>Delete interaction</button></div></div></div>}
  </div>
}
